...
TestCase {
    ...
    function test_parseCodeResponseAcceptsTheDocumentedShape() {
        const parsed = Pairing.parseCodeResponse(JSON.stringify({ code: "ABC234", expiresAt: 1750000300000, pollIntervalSeconds: 3 }));
    ...
    function test_parseCodeResponseRejectsAnythingElse() {
        [undefined, null, {}, { code: "" }, { code: "ABC234" }, { code: "ABC234", expiresAt: "soon" }, { code: "ABC234", expiresAt: 1, pollIntervalSeconds: "3" }, "ABC234"].forEach(body => {
            compare(Pairing.parseCodeResponse(JSON.stringify(body)), null, `${JSON.stringify(body)} should be rejected`);
        });
        compare(Pairing.parseCodeResponse("<html>not json</html>"), null);
    }
    ...
    function test_parsePollResponseAcceptsPending() {
        const parsed = Pairing.parsePollResponse(JSON.stringify({ status: "Pending", token: null }));
    ...
    function test_parsePollResponseAcceptsApprovedWithAToken() {
        const parsed = Pairing.parsePollResponse(JSON.stringify({ status: "Approved", token: "a-bearer-token" }));
    ...
    function test_parsePollResponseAcceptsExpiredWithANullToken() {
        const parsed = Pairing.parsePollResponse(JSON.stringify({ status: "Expired", token: null }));
    ...
    function test_parsePollResponseRejectsApprovedWithoutAToken() {
        compare(Pairing.parsePollResponse(JSON.stringify({ status: "Approved", token: null })), null);
        compare(Pairing.parsePollResponse(JSON.stringify({ status: "Approved" })), null);
    }
    ...
    function test_parsePollResponseRejectsAnUnknownStatus() {
        [undefined, null, {}, { status: "pending" }, { status: "approved", token: "x" }, { status: "Something" }, "Pending"].forEach(body => {
            compare(Pairing.parsePollResponse(JSON.stringify(body)), null, `${JSON.stringify(body)} should be rejected`);
        });
        compare(Pairing.parsePollResponse("<html>not json</html>"), null);
    }
