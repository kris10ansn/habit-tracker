> **Integration test branch:** combines PR #35's isolated test app with PR #33's adaptive layout
> and staged editor, including its PR #27 power-state renderer dependency. Deploy with
> `pnpm remarkable:deploy:test` from this worktree. Test-profile automatic rendering writes only
> a local suspend preview; the developer button can write the device suspend image once.
> Power-off and battery-empty device images remain untouched by the test profile.
