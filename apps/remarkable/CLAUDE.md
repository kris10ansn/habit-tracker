...
- **JSON I/O is asynchronous.** `Storage` serializes text reads and verified writes per path. `tests/Storage.js` provides synchronous host assertions and is never shipped. Binary image I/O still uses the existing Canvas path.
