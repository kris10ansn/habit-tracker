...
**The sibling expo client (`apps/mobile/`) is a peer, not a reference.** Never read it to decide this app's storage shape, domain model, or sync behaviour — the two clients persist deliberately different shapes (this one: month-partitioned JSON with X/O marks; mobile: SQLite rows in the backend's Outcome/Polarity shape), and neither is a spec for the other. Merge and conflict resolution belong to the backend: this client sends its state and accepts the authoritative result (see `src/js/Sync.js`). Where a client and the backend disagree, the backend wins.

**Domain docs.** Three glossaries: this app's `CONTEXT.md` (device-only terms — suspend image, suspend visibility, settings, edit mode, Server URL), the monorepo-root [`../../CONTEXT.md`](../../CONTEXT.md) (the shared Habit / Entry / X-O / polarity vocabulary every client speaks), and the [backend glossary](../backend/CONTEXT.md) (Outcome, Position, Sync, Edit-time, Tombstone — the sync contract). This app's `docs/adr/` records four deliberate decisions you must not silently undo — `0001` opt-in suspend-image writing, `0002` month-partitioned storage, `0004` viewing other months, `0005` backend-shaped entry rows (which amends `0002`). (Other ADRs were retired; code comments still citing numbers with no file behind them are stale references, not missing reading.) Read the glossaries before changing domain behaviour, and keep them current when it changes.
...
- **Persisted state uses `JsonStore.qml`.** That base owns the deferred initial load, the 200 ms debounced save, the `saved`/`saveFailed` signals, and `flushPendingSave`. A store sets `filePath` and assigns two function-property hooks: `serialize` (→ the value to write) and `applyLoaded(data)` (← fold a just-read value, handling the `Storage` MISSING/CORRUPT sentinels). Don't re-implement file I/O or the save timer per store. `SettingsStore` extends it directly; `HabitsStore` instead **composes two `JsonStore` children behind a facade** — a roster store (`data/roster.json`: alive habits as id + name + polarity + suspend visibility + createdAt + updatedAt, followed by their tombstones) and a month store (`data/YYYY-MM.json`: the viewed month's entries as flat `{ habitId, date, outcome, updatedAt, deletedAt }` rows, which is the wire format's month payload verbatim). The ListModel is the single in-memory source of truth and each child serializes a *projection* of it (`HabitsModel.toRoster` / `toMonthEntryRows`); config edits schedule the roster save, entry toggles the month save. Load is parallel, folded by id once both resolve. **Three invariants:** the model holds *alive* habits only (a delete moves the row to `habitTombstones`, so index-based APIs never drift from what the grid renders); each habit's entry rows live indexed by date on its own model row (`entriesByDate`) because that is the only per-habit reactive vehicle QML gives us — a grid-wide index would re-evaluate ~600 cell bindings per tap instead of ~31; and reads tolerate the pre-Phase-2 shapes (`Polarity.fromHabit`, `Entries.rowsFromLegacyMonth`) because reading a legacy month as empty would let the next toggle overwrite it. See [`docs/adr/0002-month-partitioned-habit-storage.md`](docs/adr/0002-month-partitioned-habit-storage.md) and [`docs/adr/0005-backend-shaped-entry-rows.md`](docs/adr/0005-backend-shaped-entry-rows.md). These `src/` stores resolve as `App.<Name>` without a `src/qmldir` entry (file-based resolution).
...
- **Verbose naming.** Prefer descriptive names for variables, properties, persisted JSON fields, and wire/domain shapes. Avoid single-letter or cryptic names except for tiny local indices; storage objects must use names like `state` and `updatedAt`, not shorthand.
