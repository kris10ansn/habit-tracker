...
- **Anything gated on a write succeeding must wait for the callback.** `SuspendRender.copyFile` and `PowerImageJobs.backup`/`restore` answer through `onDone(ok)`, propagated to the frontend by `ImageBackend`. `Main.applySuspendSetting` only turns the setting on inside that callback: enabling suspend-image writing on a backup that silently failed overwrites a stock image nothing can restore (ADR 0001). `PowerImageController` owns scheduling; `PowerImageJobs` owns captured-job completion, including signature persistence; `SuspendCanvas` only renders PNGs.
