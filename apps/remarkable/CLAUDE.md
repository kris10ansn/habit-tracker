...
A QML **habit tracker** for **reMarkable 1**, launched inside xochitl via XOVI + rm-appload.
The frontend targets Qt 5.15. Power-state images are rendered and saved by a separate native
helper process built from `tools/suspend-writer/`; deploy installs its systemd service alongside
the QML resources. Read that tool's README when changing image jobs, IPC, or deployment.
...
`make test` runs Node storage/build-profile tests and **Qt Quick Test** (`qmltestrunner-qt5`) over `tests/tst_*.qml` — one runner for both
...
- **All application file I/O is asynchronous.** `Storage` serializes reads and verified writes per path. `tests/Storage.js` contains synchronous inspection helpers for host assertions only and is never shipped. Wait for the exact expected body or a completion signal in tests.
...
`tools/suspend-writer/` is the C++ renderer and production power-image helper: it
...
ARM/Qt6, needs the SDK unpacked at `tools/suspend-writer/sdk/`). `deploy` copies the
...
- **Anything gated on a write succeeding must wait for the callback.** `PowerImageStore.backup`/`restore` answer through `onDone(ok)`, and `Main.applySuspendSetting` only turns the setting on inside that callback: enabling suspend-image writing on a backup that silently failed overwrites a stock image nothing can restore (ADR 0001).
